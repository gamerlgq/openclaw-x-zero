...
import { t as computeBackoff } from "./backoff-DZEqB7rp.js";
...
function resolveContextWindowLoadScopeKey(providerIds) {
	const scoped = providerIds?.map((value) => normalizeProviderId(value)).filter(Boolean).toSorted((left, right) => left.localeCompare(right));
...
function ensureContextWindowCacheLoaded(options) {
	...
	const pending = (async () => {
		try {
			await (await loadModelsConfigRuntime()).ensureOpenClawModelsJson(cfg, void 0, options?.providerDiscoveryProviderIds ? { providerDiscoveryProviderIds: options.providerDiscoveryProviderIds } : void 0);
		} catch {}
		try {
	...
	if (scopeKey === "*") CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = pending;
	return pending;
}
function lookupContextTokens(modelId, options) {
	if (!modelId) return;
	if (options?.allowAsyncLoad === false) primeConfiguredContextWindows();
	else ensureContextWindowCacheLoaded(options?.providerDiscoveryProviderIds ? { providerDiscoveryProviderIds: options.providerDiscoveryProviderIds } : void 0);
...
function resolveContextTokensForModel(params) {
	...
	const discoveryProviderIds = explicitProvider && ref ? [ref.provider] : void 0;
	if (ref) {
