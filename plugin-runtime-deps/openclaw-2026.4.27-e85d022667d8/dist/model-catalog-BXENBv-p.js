...
import { t as augmentModelCatalogWithProviderPlugins } from "./provider-runtime.runtime-DlUjP7F7.js";
import { T as modelSupportsInput, r as buildConfiguredModelCatalog } from "./model-selection-shared-BAGl40FK.js";
import { n as ensureOpenClawModelsJson } from "./models-config-CTWs5_xF.js";
...
function loadModelSuppression() {
	modelSuppressionPromise ??= import("./model-suppression.runtime-C8De_aKN.js");
...
async function loadModelCatalog(params) {
	...
	const providerDiscoveryProviderIds = params?.providerDiscoveryProviderIds?.map((value) => value.trim()).filter(Boolean);
	const scopedCatalog = providerDiscoveryProviderIds !== void 0 && providerDiscoveryProviderIds.length > 0;
	...
	const loadCatalog = async () => {
		...
		try {
			...
			if (!readOnly) {
				await ensureOpenClawModelsJson(cfg, void 0, scopedCatalog ? { providerDiscoveryProviderIds } : void 0);
	...
	if (!useSharedCache) return loadCatalog();
