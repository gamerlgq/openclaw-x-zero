...
import { r as isSupportedNodeVersion } from "./runtime-guard-CQFUyBrZ.js";
import { t as resolveStableNodePath } from "./stable-node-path-Ds-yFMzu.js";
import { n as getWindowsProgramFilesRoots } from "./windows-install-roots-1pOV0n75.js";
...
*/
function resolveDarwinUserBinDirs(home, env, existsSync = fs.existsSync) {
	if (!home) return [];
	const dirs = [];
...
* These are paths where npm global installs and node version managers typically place binaries.
*/
function resolveLinuxUserBinDirs(home, env, existsSync = fs.existsSync) {
	if (!home) return [];
	const dirs = [];
	addCommonEnvConfiguredBinDirs(dirs, env);
...
function getMinimalServicePathParts(options = {}) {
	...
	const userDirs = platform === "linux" ? resolveLinuxUserBinDirs(options.home, options.env, existsSync) : platform === "darwin" ? resolveDarwinUserBinDirs(options.home, options.env, existsSync) : [];
	const add = (dir) => {
		if (!dir) return;
