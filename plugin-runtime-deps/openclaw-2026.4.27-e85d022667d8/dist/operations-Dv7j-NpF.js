...
import { c as normalizeAgentId, r as buildAgentMainSessionKey } from "./session-key-C6F-NnIG.js";
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-BWYSf62L.js");
	if (operation === "start") {
...
async function loadOverviewForOperation(deps) {
	...
	const { loadCrestodianOverview } = await import("./overview-ByDARx2c.js");
...
async function formatOverviewForOperation(overview, deps) {
	...
	const { formatCrestodianOverview } = await import("./overview-ByDARx2c.js");
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "setup") {
		...
		const applyDefaultModelPrimaryUpdate = setupModel.model ? (await import("./shared-cZ5l5oPc.js")).applyDefaultModelPrimaryUpdate : void 0;
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-BgzA7dTz.js")).agentsAddCommand)({
	...
	if (operation.kind === "doctor") {
		...
		await (opts.deps?.runDoctor ?? (await import("./doctor-BQxyPl6X.js")).doctorCommand)(runtime, { nonInteractive: true });
	...
	if (operation.kind === "doctor-fix") {
		...
		const { readConfigFileSnapshot } = await import("./config/config.js");
		const before = await readConfigFileSnapshot();
		await (opts.deps?.runDoctor ?? (await import("./doctor-BQxyPl6X.js")).doctorCommand)(runtime, {
			nonInteractive: true,
			repair: true,
			yes: true
		});
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-CeRV4IiL.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-BsmD4OWl.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
	...
	if (operation.kind === "set-default-model") {
		...
		const { mutateConfigFile, readConfigFileSnapshot } = await loadConfigFileMutationHelpers();
		const before = await readConfigFileSnapshot();
		const { applyDefaultModelPrimaryUpdate } = await import("./shared-cZ5l5oPc.js");
		const result = await mutateConfigFile({
			base: "source",
			mutate: (cfg) => {
		...
		const after = await readConfigFileSnapshot();
		const { resolveAgentModelPrimaryValue } = await import("./model-input-DKdgZZQC.js");
