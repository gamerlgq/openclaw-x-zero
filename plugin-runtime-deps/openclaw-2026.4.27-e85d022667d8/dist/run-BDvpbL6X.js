...
import { t as isTruthyEnvValue } from "./env-BvnVJRcO.js";
...
import { S as setVerbose } from "./logger-B7txuiSU.js";
import "./globals-8N5C2Ov_.js";
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-DnTGmMkY.js";
import { t as createSubsystemLogger } from "./subsystem-Izr-DYZz.js";
import { t as isContainerEnvironment } from "./container-environment-BcrDUdNk.js";
import { p as resolveGatewayBindHost, t as defaultGatewayBindMode } from "./net-DDjdGYkE.js";
import { t as parsePort } from "./parse-port-BR35BIr9.js";
import { n as withProgress } from "./progress-DEutg9EU.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-JrN2nBkT.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-vGG4R79i.js";
...
let embeddedRunsModule;
let runtimeConfigModule;
let processRespawnModule;
let restartSentinelModule;
let restartModule;
let supervisorMarkersModule;
let diagnosticStabilityBundleModule;
let bundledRuntimeDepsActivityModule;
let commandQueueModule;
let runtimeInternalModule;
const loadEmbeddedRunsModule = () => embeddedRunsModule ??= import("./runs-BkwPNMTS.js");
const loadRuntimeConfigModule = () => runtimeConfigModule ??= import("./config/config.js");
const loadProcessRespawnModule = () => processRespawnModule ??= import("./process-respawn-y6bLTncK.js");
const loadRestartSentinelModule = () => restartSentinelModule ??= import("./restart-sentinel-Bf9iUmRX.js");
const loadRestartModule = () => restartModule ??= import("./restart--yFw2sFh.js");
const loadSupervisorMarkersModule = () => supervisorMarkersModule ??= import("./supervisor-markers-BpTQMuR7.js");
const loadDiagnosticStabilityBundleModule = () => diagnosticStabilityBundleModule ??= import("./diagnostic-stability-bundle-CiBSNgus.js");
const loadBundledRuntimeDepsActivityModule = () => bundledRuntimeDepsActivityModule ??= import("./bundled-runtime-deps-activity-SO6n1wod.js");
const loadCommandQueueModule = () => commandQueueModule ??= import("./command-queue-CKVxpmTJ.js");
const loadRuntimeInternalModule = () => runtimeInternalModule ??= import("./runtime-internal-CnGzHt5C.js");
...
async function runGatewayLoop(params) {
	...
	const writeStabilityBundle = async (reason, error) => {
		const { writeDiagnosticStabilityBundleForFailureSync } = await loadDiagnosticStabilityBundleModule();
	...
	const handleRestartAfterServerClose = async (restartReason) => {
		...
		const { respawnGatewayProcessForUpdate, restartGatewayProcessWithFreshPid } = await loadProcessRespawnModule();
	...
	const request = (action, signal, restartReason) => {
		...
		(async () => {
			...
			try {
				if (isRestart) {
					const [{ abortEmbeddedPiRun, getActiveEmbeddedRunCount, waitForActiveEmbeddedRuns }, { getActiveBundledRuntimeDepsInstallCount, waitForBundledRuntimeDepsInstallIdle }, { getActiveTaskCount, markGatewayDraining, waitForActiveTasks }] = await Promise.all([
	...
	const onSigusr1 = () => {
		...
		(async () => {
			const { consumeGatewaySigusr1RestartAuthorization, isGatewaySigusr1RestartExternallyAllowed, markGatewaySigusr1RestartHandled, peekGatewaySigusr1RestartReason, scheduleGatewaySigusr1Restart } = await loadRestartModule();
	...
	try {
		const onIteration = createRestartIterationHook(async () => {
			const [{ resetAllLanes }, { resetGatewayRestartStateForInProcessRestart }, { reloadTaskRegistryFromStore }] = await Promise.all([
				loadCommandQueueModule(),
				loadRestartModule(),
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-HFc76KRb.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-epox9_Yb.js");
...
async function readGatewayStartupConfig(params) {
	...
	if (snapshot?.exists && !snapshot.valid) {
		...
		if (await params.startupTrace.measure("cli.config-recovery", () => recoverConfigFromLastKnownGood({
			...
			try {
				const { writeRestartSentinel } = await import("./restart-sentinel-Bf9iUmRX.js");
...
async function runGatewayLoopWithSupervisedLockRecovery(params) {
	...
	for (;;) try {
		...
		if (!(err instanceof GatewayLockError && typeof err.message === "string" && err.message.includes("gateway already running"))) throw err;
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-CiBSNgus.js");
...
async function runGatewayCommand(opts) {
	...
	const { formatFutureConfigActionBlock, resolveFutureConfigActionBlock } = await import("./future-version-guard-BrxgVzqF.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-FLUx2q0b.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-DZee3ezi.js");
	...
	const { resolveGatewayAuth } = await import("./auth-Birk_1EA.js");
	...
	try {
		const { detectRespawnSupervisor } = await import("./supervisor-markers-BpTQMuR7.js");
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-CLbPRTc0.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-DwG2z0U0.js");
