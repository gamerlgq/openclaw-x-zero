...
import { a as parseSemver } from "./runtime-guard-CQFUyBrZ.js";
import { t as resolveStableNodePath } from "./stable-node-path-Ds-yFMzu.js";
import { r as runCommandWithTimeout } from "./exec-2P596n4G.js";
import { m as trimLogTail } from "./restart-sentinel-C0QvDWy0.js";
import { t as BUNDLED_RUNTIME_SIDECAR_PATHS } from "./runtime-sidecar-paths-x_1yh4vX.js";
import { t as applyPathPrepend } from "./path-prepend-CoF_tLHM.js";
import { a as resolveControlUiDistIndexPathForRoot, r as resolveControlUiDistIndexHealth } from "./control-ui-assets-BPDXvwAq.js";
import { f as isBetaTag, l as DEV_BRANCH, n as compareSemverStrings, p as isStableTag, s as detectPackageManager, u as channelToNpmTag } from "./update-check-CreMMaou.js";
...
const DIST_JS_IMPORT_SPECIFIER_PATTERN = /\b(?:import|export)\s+(?:(?:[^'"()]*?\s+from\s+)|)["'](?<staticSpecifier>[^"']+)["']|\bimport\s*\(\s*["'](?<dynamicSpecifier>[^"']+)["']\s*\)/gu;
...
function resolveDistImportTarget(importer, specifier, actualFiles) {
	const normalizedSpecifier = normalizeModuleSpecifierTarget(specifier);
	const base = normalizeRelativePath(path.posix.normalize(path.posix.join(path.posix.dirname(importer), normalizedSpecifier)));
	return [
		base,
		`${base}.js`,
		`${base}.mjs`,
		...
		`${base}/index.mjs`,
		`${base}/index.cjs`
	].find((candidate) => actualFiles.has(candidate)) ?? null;
}
async function collectPackageDistImportReferenceErrors(packageRoot) {
	const actualFiles = new Set(await collectPackageDistInventory(packageRoot));
	const jsFiles = PACKAGE_DIST_IMPORT_REFERENCE_ENTRYPOINTS.filter((relativePath) => actualFiles.has(relativePath));
	const errors = [];
	await Promise.all(jsFiles.map(async (relativePath) => {
		let source;
		try {
			source = await fs$1.readFile(path.join(packageRoot, relativePath), "utf8");
		} catch {
		...
		for (const match of source.matchAll(DIST_JS_IMPORT_SPECIFIER_PATTERN)) {
			...
			if (resolveDistImportTarget(relativePath, specifier, actualFiles)) continue;
...
async function collectInstalledPackageDistErrors(params) {
	...
	if (inventoryFiles !== null) {
		...
		if (supplementalCriticalPaths.length === 0) return [...inventoryErrors, ...importErrors];
		return [
			...inventoryErrors,
			...importErrors,
