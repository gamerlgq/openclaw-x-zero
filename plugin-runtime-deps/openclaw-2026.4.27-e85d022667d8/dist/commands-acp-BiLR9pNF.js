...
import { a as requireGatewayClientScopeForInternalChannel } from "./command-gates-CgCQllYj.js";
import { C as stopWithText, b as resolveAcpAction, d as COMMAND, x as resolveAcpHelpText } from "./shared-DVeMKk53.js";
...
async function loadAcpActionHandler(action) {
	if (action === "spawn" || action === "cancel" || action === "steer" || action === "close") {
		lifecycleHandlersPromise ??= import("./lifecycle-Vuz7FEJI.js");
	...
	if (action === "status" || action === "set-mode" || action === "set" || action === "cwd" || action === "permissions" || action === "timeout" || action === "model" || action === "reset-options") {
		runtimeOptionHandlersPromise ??= import("./runtime-options-BkiM-ZkI.js");
	...
	diagnosticHandlersPromise ??= import("./diagnostics-CpRHwhnq.js");
