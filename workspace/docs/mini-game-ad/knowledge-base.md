...
- [x] 腾讯广告 API 实际调用示例（access_token 获取、创建广告组等） → `api-examples.md`
- [x] 巨量引擎 API 实际调用示例 → `api-examples.md`
- [x] 两平台数据归因与回传配置细节 → `attribution-callback.md`
- [x] 素材审核规则与常见拒审原因 → `creative-review.md`
- [x] 出价策略优化实操（oCPM/ROI 出价调优） → `bidding-strategy.md`
- [x] 跨平台投放策略（微信+抖音联合买量） → `cross-platform-strategy.md`
